import os
from os import environ  # Assuming environ is used as in the error

# Configuration class or direct variables
class Config:  # If using a class, ensure proper indentation
    BOT_TOKEN = environ.get('BOT_TOKEN','7860780283:AAFt9MlBsWL7_qVZi87cQL01WbEaqOB8mV8')
    API_ID = int(environ.get('API_ID', '12618934'))
    API_HASH = environ.get('API_HASH','49aacd0bc2f8924add29fb02e20c8a16')
    MONGO_URI = environ.get('MONGO_URI','mongodb+srv://saidalimuhamed88:iladias2025@cluster0.qt4dv.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0')
    DB_NAME = environ.get('DB_NAME', 'Cluster0')
    AUTH_CHANNEL = environ.get('AUTH_CHANNEL','-1002256041072')
    LOG_CHANNEL = environ.get('LOG_CHANNEL','-1002467149516')
    OWNER_ID = int(environ.get('OWNER_ID', '5032034594'))
    AUTO_DELETE_TIME = int(environ.get('AUTO_DELETE_TIME', '60'))

# If not using a class, ensure no stray block starters
BOT_TOKEN = environ.get('BOT_TOKEN','7860780283:AAFt9MlBsWL7_qVZi87cQL01WbEaqOB8mV8')
API_ID = int(environ.get('API_ID', '12618934'))
API_HASH = environ.get('API_HASH','49aacd0bc2f8924add29fb02e20c8a16')
MONGO_URI = environ.get('MONGO_URI','mongodb+srv://saidalimuhamed88:iladias2025@cluster0.qt4dv.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0')
DB_NAME = environ.get('DB_NAME', 'Cluster0')
AUTH_CHANNEL = environ.get('AUTH_CHANNEL','-1002256041072')
LOG_CHANNEL = environ.get('LOG_CHANNEL','-1002467149516')
OWNER_ID = int(environ.get('OWNER_ID', '5032034594'))
AUTO_DELETE_TIME = int(environ.get('AUTO_DELETE_TIME', '60'))


PORT = int(environ.get('PORT', 8000))
#
